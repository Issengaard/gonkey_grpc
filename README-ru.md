...
### Matcher `$matchArrayLen`

`$matchArrayLen(...)` проверяет длину массива в ответе без перечисления
самих элементов. Удобно для справочных эндпоинтов, содержимое которых
...
```yaml
    response:
        200: |
          {
            "permissions": "$matchArrayLen(55)",         # ровно 55 элементов
            "roles": "$matchArrayLen(min=1)",            # хотя бы один
            "items": "$matchArrayLen(min=3,max=10)",     # включительный диапазон
            "events": "$matchArrayLen(max=100)"          # только верхняя граница
          }
```

Пробелы вокруг `=` и `,` допускаются. Содержимое массива не проверяется —
если нужно проверить и элементы, используйте обычный `expected: [...]`.
Применение `$matchArrayLen` к не-массиву даёт ошибку
`matchArrayLen requires array`.
